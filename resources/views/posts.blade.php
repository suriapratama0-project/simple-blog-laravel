        ...
        <div class="mx-auto max-w-screen-md sm:text-center">
            <form>
                ...
                <div class="items-center mx-auto mb-3 space-y-4 max-w-screen-sm sm:flex sm:space-y-0">
                    ...
                    <div>
                        <button type="submit"
                            class="py-3 px-5 w-full text-sm font-medium text-center text-white border cursor-pointer bg-blue-700 border-blue-600 sm:rounded-none sm:rounded-r-lg hover:bg-primary-800 focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Search</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div class="py-4 px-4 mx-auto max-w-screen-xl lg:py-4 lg:px-0">
        <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
                ...
                in
                <a class="hover:underline text-base text-gray-500" href="/categories/{{ $post->category->slug }}">{{ $post->category->category_names }}</a> | {{ $post->created_at->diffForHumans() }}
            </div>
                ...
                <article
                    ...
                    <div class="flex justify-between items-center mb-5 text-gray-500">
                        <a class="hover:underline" href="/posts?category={{ $post->category->slug }}">
                            <span
                                ...
                                {{ $post->category->category_names }}
                            </span>
                        </a>
                        <span class="text-sm">{{ $post->created_at->diffForHumans() }}</span>
                    </div>
                    <h2 class=" hover:underline mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><a
                            href="/posts/{{ $post['slug'] }}">{{ $post['tittle'] }}</a></h2>
                    ...
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-4">
                            ...
                            <a class="hover:underline" href="/posts?author={{ $post->author->username }}">
